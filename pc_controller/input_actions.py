...
def start_anti_afk() -> CommandResult:
    ...
    if _afk_active:
        return CommandResult(False, "Anti-AFK уже работает.")
    ...
    def _afk_loop():
        import random
        while _afk_active:
            # Случайные действия, чтобы система точно не считала нас AFK
...
def stop_anti_afk() -> CommandResult:
    global _afk_active
    if not _afk_active:
        return CommandResult(False, "Anti-AFK не запущен.")
    _afk_active = False
...
class AutoAcceptService:
    ...
    def start(self, config: AutoAcceptConfig, on_match: Callable[[str], None], on_error: Callable[[str], None],
              on_finish: Callable[[str], None]) -> CommandResult:
        if self._active:
            return CommandResult(False, 'AutoAccept is already running.')
        if not config.template_dir.exists():
            return CommandResult(False, 'Template directory does not exist.')
        templates = [p for p in config.template_dir.iterdir() if
                     p.is_file() and p.suffix.lower() in {'.png', '.jpg', '.jpeg', '.bmp'}]
        if not templates:
            return CommandResult(False, 'No templates found in the directory.')

        self._active = True
        self._stop_event.clear()
        self._thread = threading.Thread(target=self._run, args=(config, templates, on_match, on_error, on_finish),
                                        daemon=True)
        self._thread.start()
    ...
    def stop(self) -> CommandResult:
        if not self._active:
            return CommandResult(False, 'AutoAccept is not running.')
    ...
    def _run(self, config: AutoAcceptConfig, templates: list[Path], on_match: Callable[[str], None],
             on_error: Callable[[str], None], on_finish: Callable[[str], None]) -> None:
        _ensure_pyautogui()
